...
function fill_problem_table(tc::PISPtimeConfig)
    start_date = DateTime(2025, 1, 7, 0, 0, 0)
    step_ = Day(7) 
...
# ============================================ #
# CSV format
PISP.PISPwritedataCSV(ts, normpath(@__DIR__,"hydro-test/csv"))                    # Time-static data
PISP.PISPwritedataCSV(tv, normpath(@__DIR__,"hydro-test/csv/schedule-1w"))    # Time-varying data (schedules)

# Arrow format
PISP.PISPwritedataArrow(ts, normpath(@__DIR__,"hydro-test/arrow"))                    # Time-static data
PISP.PISPwritedataArrow(tv, normpath(@__DIR__,"hydro-test/arrow/schedule-1w"))    # Time-varying data (schedules)